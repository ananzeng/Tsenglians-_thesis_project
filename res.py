...
parser.add_argument('--experiment_name', default='Official_exp_voc_L2G', type=str)
...
if __name__ == '__main__':
    ...
    train_lst = 'data/voc12/train_cls.txt'
    root_folder = 'data/voc12/'
    im_lst, label_lst = load_dataset(train_lst)
    
    atten_path = './runs/{}/ms/attention'.format(args.experiment_name)
    save_path = './runs/{}/ms/accu_att_zoom'.format(args.experiment_name)
    ...
    for i in range(len(im_lst)):
        ...
        for label in im_labels:
            att_name = '{}/{}_{}.png'.format(atten_path, im_lst[i], label)
