...
def get_arguments():
    ...
    parser.add_argument("--cda", dest='cda', action='store_true', default=False)
    parser.add_argument("--cda_tr", type=float, default=0.2)
...
def get_resnet38_model(args):
    ...
    if len(args.load_checkpoint) > 0:
        optimizer = optim.SGD([
            ...
            {'params': param_groups_local[3], 'lr': 20 * args.lr}], momentum=0.9, weight_decay=args.weight_decay,
            nesterov=True)
        opt_weights_dict = optimizer.state_dict()
...
def train(args):
    ...
    while current_epoch < total_epoch:
        ...
        for idx, dat in enumerate(train_loader):
            ##########增加的##########
            if args.cda:
                img, crop_imgs, crop_sals, boxes, label, local_label, img_name, ori_image, ori_crop_images, ori_crop_sals, ori_boxes, ori_label, ori_label_local, ori_img_name = dat
                img=torch.cat([img,ori_image],0)
                crop_imgs=torch.cat([crop_imgs,ori_crop_images],0)
                crop_sals=torch.cat([crop_sals,ori_crop_sals],0)
                boxes=torch.cat([boxes,ori_boxes],0)
                label=torch.cat([label,ori_label],0)
                local_label=torch.cat([local_label,ori_label_local],0)
            else:
...
if __name__ == '__main__':
    ...
    os.makedirs(args.snapshot_dir[:-7]+"/logs/", exist_ok = True)
    logging.basicConfig(filename = args.snapshot_dir[:-7]+"/logs/result.log", level = logging.INFO)
    logging.debug("start L2G train")
    train(args)
