...
def train(config_path, cuda):
    ...
    print(dataset)
    print(CONFIG.DATASET.LABEL_DIR)
    print(CONFIG.EXP.ID)
    # DataLoader
    ...
    criterion = FocalLoss(ignore_index=CONFIG.DATASET.IGNORE_LABEL, size_average=True)
    criterion.to(device)
...
def test(config_path, model_path, cuda):
    ...
    makedirs(save_dir)
    iteration = model_path.split('_')[-1][:-len('.pth')]
    save_path = os.path.join(save_dir, f"scores_{iteration}.json")
