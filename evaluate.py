...
parser.add_argument('--experiment_name', default='ResNet50@Puzzle@3090@optimal@train@scale=0.5,1.0,1.5,2.0', type=str)
parser.add_argument("--domain", default='train', type=str)
parser.add_argument("--threshold", default=None, type=float)
parser.add_argument('--gt_dir', default='./data/voc12/SegmentationClass', type=str)
parser.add_argument('--mode', default='npy', type=str) # png
parser.add_argument('--max_th', default=0.50, type=float)

args = parser.parse_args()

predict_folder = './runs/{}/cam_npy_aff/'.format(args.experiment_name)
gt_folder = args.gt_dir

args.list = './data/voc12/' + args.domain + '1.txt' #因為L2G train_aug.txt不符規格 所以我複製一個新的train_aug1.txt
...
def compare(start, step, TP, P, T, threshold, name_list):
...
def do_python_eval(gt_folder, name_list, num_cls=21, num_cores=8, threshold=0):
    TP = []
    P = []
    T = []
    for i in range(num_cls):
    ...
    for i in range(num_cores):
        p = multiprocessing.Process(target=compare, args=(i,num_cores,TP,P,T,threshold,name_list))
        p.start()
        p_list.append(p)
    for p in p_list:
        p.join()
...
if __name__ == '__main__':
    ...
    if args.mode == 'png':
        loglist = do_python_eval(args.gt_dir, name_list, 21)
        ...
        for th in th_list:
            loglist = do_python_eval(args.gt_dir, name_list, 21, threshold=th)
        ...
        if args.threshold is None:
            ...
            for th in th_list:
                loglist = do_python_eval(args.gt_dir, name_list, 21, threshold=th)
                print('Th={:.2f}, mIoU={:.3f}%, FP={:.4f}, FN={:.4f}'.format(th, loglist['mIoU'], loglist['fp_all'], loglist['fn_all']))
            ...
            loglist = do_python_eval(args.gt_dir, name_list, 21, threshold=args.threshold)
