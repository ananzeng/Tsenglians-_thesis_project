...
from .imutils import ResizeShort, RandomResizeLong, get_random_crop_box, crop_with_box, data_aug, data_aug_plus
import os
...
# multi scale inference
class VOCDatasetMSF_l2g(Dataset): #!testing用
...
def train_l2g_sal_crop_data_loader(args, test_path=False, segmentation=False): #!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    ##########增加的##########
    if args.cda:
        img_train = VOCDataset_l2g_sal_crop_aug_pairwise_batchsize2_plus(args.train_list, root_dir=args.img_dir, sal_dir=args.sal_dir, input_size=input_size, crop_size=crop_size,
                                       num_classes=args.num_classes, transform=tsfm_train, test=False, patch_size=args.patch_size, cda_tr = args.cda_tr) #加了CDA
    else:
...
class VOCDataset_l2g_sal_crop_aug(Dataset):
    def __init__(self, datalist_file, root_dir, sal_dir='Sal', input_size=448, crop_size=224, num_classes=20, patch_size=4,
                 transform=None, test=False):
        self.root_dir = root_dir
        self.testing = test
    ...
    def __getitem__(self, idx):
        ...
        label = self.label_list[idx]
        label_index=np.arange(len(label))  #0-20
        label_index=label_index[label==0]  #找不是圖片中那類的
        fg_label=np.random.choice(label_index)     #隨便取label(要貼上的)
        #print("fg class:")
        #print("fg_label", fg_label)
        #numpy.ndarray
        fg_name=np.random.choice(np.load('sem_seg_fg/seg_fg_dirs.npy', allow_pickle=True).item()[fg_label]) #用label隨便取檔名
        #print("fg_name", fg_name)
        fg_pack=np.load(os.path.join('sem_seg_fg',str(fg_label),fg_name), allow_pickle=True).item()
        fg_img=fg_pack['img']
        fg_seg=fg_pack['seg']
        fg_sal=fg_pack['sal']
        #print("fg_name", fg_name)
        #print("fg_seg.shape", fg_seg.shape)
        aug_image, aug_sal=data_aug(image, fg_img, fg_seg, sal, fg_sal, fg_name) #CDA/misc/imutils.py
        #print("aug_image.shape", aug_image.size)
...
class VOCDataset_l2g_sal_crop_aug_pairwise_batchsize2(Dataset):
    def __init__(self, datalist_file, root_dir, sal_dir='Sal', input_size=448, crop_size=224, num_classes=20, patch_size=4,
                 transform=None, test=False):
        self.root_dir = root_dir
        self.testing = test
    ...
    def __getitem__(self, idx):
        for number in range(2):
            ...
            if number == 0:
                ori_image = image
                ori_crop_images = crop_images
                ori_crop_sals = crop_sals
                ori_boxes = boxes
                ori_label = label
                ori_label_local = label_local
                ori_img_name = img_name
            ################要加的################
            
        ################要加的################
        """
        con_image = torch.cat([ori_image, image],0)
        con_crop_images = torch.cat([ori_crop_images, crop_images],0)
        con_crop_sals = torch.cat([ori_crop_sals, crop_sals],0)
        con_boxes = torch.cat([ori_boxes, boxes],0)
        #print("ori_label", ori_label.type)
        #print("label", label.type)
        #np.concatenate([x, y], axis=1)
        con_label = np.concatenate([ori_label, label], axis=0)
        #con_label = torch.cat([ori_label, label],0)
        con_label_local = np.concatenate([ori_label_local, label_local], axis=0)
        #con_label_local = torch.cat([ori_label_local, label_local],0)
        con_img_name = []
        con_img_name.append(ori_img_name)
        con_img_name.append(img_name)
        #con_img_name = torch.cat([ori_img_name, img_name],0)
        print("con_image.shape", con_image.shape)
        """
        return image, crop_images, crop_sals, boxes, label, label_local, img_name, ori_image, ori_crop_images, ori_crop_sals, ori_boxes, ori_label, ori_label_local, ori_img_name
        ################要加的################
        """
        return image, crop_images, crop_sals, boxes, label, label_local, img_name
        """
        #con_imgs=torch.cat([img,aug_img],0) #add
        #con_labels=torch.cat([label,aug_label],0) #add
        #CDA是這樣加的
    def box_generation(self):
        max_range = self.input_size - self.crop_size
        assert(self.patch_size >= 4)
...
class VOCDataset_l2g_sal_crop_aug_pairwise_batchsize2_plus(Dataset):
    def __init__(self, datalist_file, root_dir, sal_dir='Sal', input_size=448, crop_size=224, num_classes=20, patch_size=4, cda_tr=0.2,
                 transform=None, test=False):
        self.root_dir = root_dir
        self.testing = test
        self.datalist_file = datalist_file
    ...
    def __getitem__(self, idx):
        for number in range(2):
            ...
            if number == 1:
                ...
                aug_image_list = []
                aug_sal_list = []
                overlap_pixel_percent_list = []
                
                for i in range(10):
                    aug_image, aug_sal, overlap_pixel_percent=data_aug_plus(image, fg_img, fg_seg, sal, fg_sal, fg_name) #CDA/misc/imutils.py
                    aug_image_list.append(aug_image)
                    aug_sal_list.append(aug_sal)
                    overlap_pixel_percent_list.append(overlap_pixel_percent)
                    #print("self.cda_tr",self.cda_tr)
                    if overlap_pixel_percent<self.cda_tr:
                        break
                        
                aug_image = aug_image_list[np.argmin(overlap_pixel_percent_list)]
                aug_sal = aug_sal_list[np.argmin(overlap_pixel_percent_list)]
                #print("aug_image.shape", aug_image.size)
                #print("image.shape", image.size)
                image = aug_image
                sal = aug_sal
